...
- [ ] Smoke test прошёл
- [ ] Ворктри чистый (см. ниже)

---

## Чистый ворктри перед DONE

Перед тем как репортить DONE — всегда проверяй что ворктри чистый:

```bash
git status
# Должно быть: "nothing to commit, working tree clean"
# Если нет — либо закоммить, либо объясни почему файлы не коммитятся
```

**Правило:** если `git status` показывает что-то кроме `nothing to commit` — работа не завершена.

Варианты:
- Незакоммиченные изменения → `git add -p && git commit -m "..."`
- Новые файлы которые нужны → `git add <file> && git commit`
- Артефакты сборки / временные файлы → убедись что они в `.gitignore`

Чистый ворктри = все изменения либо закоммичены, либо явно проигнорированы. Это финальная проверка перед DONE.
