...
При каждом пробуждении — **первое действие — переставь alarm**:
```bash
# БЕЗУСЛОВНО, не проверяя существующий.
# Alarm one-shot: после срабатывания он автоматически удаляется — это нормально.
# Просто пиши поверх с актуальным контекстом.
curl -s -X POST "http://localhost:3000/api/sessions/YOUR_SESSION_ID/alarm" \
  -H "Content-Type: application/json" \
  -d '{"message": "Ты координатор. Итерация N. Текущий шаг: implement/review. Следующее действие: ...", "check_after_ms": 600000}'
```
