...
## Rule #0 — Commit before passing

**Any code another session needs must be committed first.**

Not in git → doesn't exist from another session's perspective. Uncommitted changes are invisible across session boundaries.
...
Use branches for in-progress work — they're visible to other sessions via `git branch`.
