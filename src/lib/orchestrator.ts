...
function buildDelegationPrompt(replyToSessionId: string, delegationTask: string | undefined, base: string): string {
  const lines = [
    ...
    ``,
    `Before reporting back, you MUST persist your work:`,
    `  - Code changes → commit to a branch (even partial: "git add -p && git commit -m 'wip: <what>'")`,
    ...
    `Without -N, curl buffers the stream and you get no output — you won't know if the session started.`,
    `  CHILD_ID=$(curl -s -N -X POST "http://localhost:3000/api/sessions/start" -H "Content-Type: application/json" -d '{...}' | grep -o '"session_id":"[^"]*"' | head -1 | sed 's/.*"session_id":"\\([^"]*\\)".*/\\1/')`,
    `  [ -z "$CHILD_ID" ] && echo "ERROR: spawn failed" || echo "OK: $CHILD_ID"`,
    `If CHILD_ID is empty — the session did not start. Retry before proceeding.`,
    ``,
    `When your task is complete — success or failure — report back:`,
    ``,
    `On success:`,
    `  curl -s -X POST "${base}/api/sessions/${replyToSessionId}/reply" \\`,
    `    -H "Content-Type: application/json" \\`,
    `    -d '{"message": "DONE: <summary> | committed: <branch/commit> | files: <what was saved>"}'`,
    ``,
    `On failure:`,
    `  curl -s -X POST "${base}/api/sessions/${replyToSessionId}/reply" \\`,
    `    -H "Content-Type: application/json" \\`,
    `    -d '{"message": "FAILED: <reason> | partial work committed: <yes/no, branch if yes>"}'`,
    ``,
    `Do NOT finish without calling one of these AND committing your work. The parent session is waiting.`,
