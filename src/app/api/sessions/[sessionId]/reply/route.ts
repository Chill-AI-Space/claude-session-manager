...
import { buildCodexResumeArgs, buildCodexResumeShellCommand } from "@/lib/codex-command";
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          const winArgs = buildCodexResumeArgs({
            sessionId,
            message,
            skipPermissions,
          });
          const { terminal } = await openInTerminal(
            ...
            process.platform === "win32"
              ? { executable: bin, args: winArgs, preferredTerminal }
              : undefined
          );
